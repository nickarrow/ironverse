# Devil's Chain
**Location:** Expanse

`iv-oracle:Sector Trouble|45|Notorious pirate clan preys on starships|oracle_rollable:starforged/campaign_launch/sector_trouble` 

**Known Settlements**
`iv-entity-create:Settlement|Pinnacle|Pinnacle.md` is a deep space station. 
`iv-entity-create:Settlement|Reck|Reck.md` is on `iv-entity-create:Planet|Ackriss-2|Ackriss-2.md`

**Known Factions**
[[Risen Union]] is present, but to what extent is unknown. 
[[Circle of the Elder Stars]] may or may not be present, they are certainly not based here. 

**Sector Map**

![[Map - Devil's Chain.excalidraw]]